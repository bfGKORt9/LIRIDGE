                        ...
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              // 【修正ポイント】真の円運動（外積ベクトル）を検知するロジックを搭載
                              GestureDetector(
                                onPanUpdate: (details) {
                                  const double centerXY = 87.5; // コンテナ175の半分の中心点
                                  Offset current = details.localPosition;
                                  Offset previous = current - details.delta;
                                  
                                  double v1x = previous.dx - centerXY;
                                  double v1y = previous.dy - centerXY;
                                  double v2x = current.dx - centerXY;
                                  double v2y = current.dy - centerXY;
                                  
                                  // クロス積による回転方向と速度の算出
                                  double crossProduct = v1x * v2y - v1y * v2x;
                                  
                                  // 感度（数値が大きいほど少しの指の動きで大きく回る）
                                  double sensitivity = 0.025; 
                                  double delta = crossProduct * sensitivity;
