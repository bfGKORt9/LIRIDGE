                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        icon: Icon(Icons.vibration, color: ledColor.withOpacity(0.6)), 
                        onPressed: () {
...
class RotaryDial extends StatefulWidget {
  ...
  const RotaryDial({
    super.key, required this.mode, required this.value, required this.ledColor, required this.onChanged,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onPanStart: (details) {
        ...
        _lastAngle = math.atan2(localPos.dy - center, localPos.dx - center);
      },
      onPanUpdate: (details) {
        final box = context.findRenderObject() as RenderBox;
        final localPos = box.globalToLocal(details.globalPosition);
        
        double currentAngle = math.atan2(localPos.dy - center, localPos.dx - center);
        ...
        double deltaValue = deltaAngle * (100.0 / (2 * math.pi));
        double newValue = (widget.value + deltaValue).clamp(0.0, 100.0);
      ...
      child: Stack(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              boxShadow: [BoxShadow(color: widget.ledColor.withOpacity(0.35), blurRadius: 25, spreadRadius: 3)],
            ),
          ),
          Container(
            ...
            child: CustomPaint(painter: MainDialPainter(mode: widget.mode, value: widget.value)),
          ),
          Positioned(
            bottom: 12,
            child: Text("SET TO ${widget.value.toInt()}", style: TextStyle(fontSize: 10, color: widget.ledColor, fontWeight: FontWeight.bold, letterSpacing: 1)),
          )
        ],
      ),
    );
